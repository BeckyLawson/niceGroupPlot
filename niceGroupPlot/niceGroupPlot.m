...
d.dotsize=1;
for c=1:length(unique(cell2mat(cond)))
...
'color',  mat2cell(colors(p.Results.col1, : ), 1), 'xValues', unique(cell2mat(cond))-0.2, 'groups', p.Results.cond{1})
% for group 2
violinPlot(p.Results.data{2}, 'histOri', 'right', 'widthDiv', [2 1], 'showMM', 0, ...
'color',  mat2cell(colors(p.Results.col2, : ), 1), 'xValues', unique(cell2mat(cond))+0.3, 'groups', p.Results.cond{2})
hold on
...
% for group 1
violinPlot(p.Results.data{1}, 'histOri', 'left', 'widthDiv', [2 1], 'showMM', 0, ...
'color',  mat2cell(colors(p.Results.col1, : ), 1), 'xValues', unique(cell2mat(cond))-0.05, 'groups', p.Results.cond{1})
% for group 2
violinPlot(p.Results.data{2}, 'histOri', 'right', 'widthDiv', [2 1], 'showMM', 0, ...
